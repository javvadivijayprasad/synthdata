...
import { toCsv, toSqlFile, toSqliteDb } from '../src/export.js';
import { authorPlan, detectProvider, resolveKey } from '../src/llm.js';

function parseArgs(argv) {
  const flags = { _: [] };
  const map = { '-s': 'schema', '-c': 'case', '-P': 'plan', '-o': 'out', '-k': 'key',
                '-p': 'provider', '-m': 'model', '--csv': 'csv', '--sql': 'sql',
                '--seed': 'seed', '--rows': 'rows', '--schema': 'schema', '--case': 'case', '--plan': 'plan',
...
async function cmdGenerate(flags, planText = null) {
  ...
  if (!flags.out && !flags.csv && !flags.sql) die('give an output: -o out.db and/or --csv dir / --sql file');
  const ddl = fs.readFileSync(flags.schema, 'utf-8');
...
async function cmdAuto(flags) {
  ...
  const ddl = fs.readFileSync(flags.schema, 'utf-8');
  const tables = parseDDL(ddl);
  const rows = flags.rows != null ? +flags.rows : 100;
...
usage:
  ...
  synthdata generate -s schema.sql -P plan.yaml -o out.db [--csv dir] [--sql file] [--seed 42]
